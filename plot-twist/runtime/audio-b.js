...
(function(){
  ...
  function setMuted(value){muted=!!value;Engine.storage.write('muted',muted);if(master)ramp(master.gain,muted?0:volume,0.35);if(!muted&&ctx?.state==='running'&&(!channel||blocked))playFor(phase,true);notify();}
  ...
  async function fanfare(key){
    ...
    try {
      const buffer=await load('audio-b/fanfare.mp3');
  ...
  Engine.audio={unlock,playFor,fanfare,setMuted,setVolume,isMuted:()=>muted,volume:()=>volume,needsGesture:()=>!ctx||ctx.state!=='running'||blocked,stop:()=>{phase='';generation++;channels.forEach(ch=>retire(ch));channel=null;}};
})();
