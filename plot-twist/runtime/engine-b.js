/* Game Ecosystem B — shared transport, identity and round lifecycle. No dependencies. */
(function (global) {
  'use strict';
  const settings = global.GAME_B_CONFIG || {};
  const query = new URLSearchParams(location.search);
  const preview = query.get('mode') !== 'live' && settings.preview !== false;
  const mode = preview ? 'preview' : 'live';
  const prefix = 'games-b:' + mode + ':';
  ...
  const read = (key, fallback = null) => { try { return JSON.parse(localStorage.getItem(prefix + key)) ?? fallback; } catch (_) { return fallback; } };
  const write = (key, value) => { try { localStorage.setItem(prefix + key, JSON.stringify(value)); } catch (_) {} };
  const esc = s => String(s ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const delay = ms => new Promise(resolve => setTimeout(resolve, ms));
  function configure(opts) {
    config = {...opts, appsScriptUrl: settings.appsScriptUrl || opts.appsScriptUrl};
  }
  function demoGet(action, args) {
    if (action === 'getStudent') {
      const names = {'1001':'Alex','1002':'Sam','1003':'Riley'};
      return names[args.id] ? {found:true, studentId:args.id, displayName:names[args.id], period:'Preview'} : {found:false};
    }
    if (action === 'getGame') return read('game:' + args.code, {exists:false});
    const tag = action === 'getPlayers' ? 'player:' : 'run:';
    const rows = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (!key.startsWith(prefix + tag)) continue;
      const row = read(key.slice(prefix.length));
      if (!row || (args.code && row.code !== args.code) || (args.round !== undefined && String(row.round) !== String(args.round)) || (args.gameType && row.gameType !== args.gameType)) continue;
      rows.push(row);
    }
    return rows.sort((a,b) => b.score-a.score).slice(0,args.limit || 100);
  }
  function demoPost(action, body) {
    let game = read('game:' + body.code);
    if (action === 'createGame') {
      if (game) throw new Error('code_exists');
      const old = read('game:' + body.retireCode);
      if (old) write('game:' + body.retireCode, {...old, retired:true});
      game = {exists:true, code:body.code, gameType:body.gameType, round:0, durationSec:0, countdownEndsAt:0, gameEndsAt:0, retired:false};
      write('game:' + body.code, game);
    } else if (action === 'startRound') {
      if (!game) throw new Error('not_found');
      write('game:' + body.code, {...game, ...body, startedAt:Date.now()});
    } else if (action === 'endRound') {
      if (!game) throw new Error('not_found');
      write('game:' + body.code, {...game, countdownEndsAt:Math.min(game.countdownEndsAt,Date.now()), gameEndsAt:Date.now()});
    } else {
      if (!game || game.retired) throw new Error('not_found');
      const tag = action === 'updatePlayer' ? 'player:' : 'run:';
      write(tag + body.code + ':' + body.round + ':' + body.studentId, {...body,updatedAt:Date.now()});
    }
    return {ok:true,code:body.code};
  }
  async function request(action, args, post) {
    if (preview) { await delay(160); return post ? demoPost(action,args) : demoGet(action,args); }
  ...
  function startPolling(code, intervalMs, onUpdate) {
    ...
    async function tick() {
      ...
      finally { if(token===pollToken) pollTimer=setTimeout(tick, preview ? 700 : Math.min(8000,(intervalMs||2000)*(1+failures))+Math.random()*700); }
    }
    tick();
  }
  ...
  global.Engine={
    configure,generateCode,COUNTDOWN_MS:5000,WAITING_ROUND:0,preview,mode,
    ...
    players:{pushProgress,pushWaitingPresence:fields=>pushProgress({round:0,score:0,streak:0,bestStreak:0,correct:0,attempted:0,...fields}),saveRun,retrySave,getPlayers:(code,round)=>get('getPlayers',{code,round})},
