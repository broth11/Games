(function(){
  'use strict';
  const E=Engine,B=window.GAME_BRAND,esc=E.esc,q=new URLSearchParams(location.search);
  E.configure({gameType:B.gameType,music:{idle:'../audio-release/'+B.slug+'-idle.mp3',live:'../audio-release/'+B.slug+'-live.mp3'}});
  const shell=document.getElementById('hostMain'),status=document.getElementById('hostStatus');
  let code='',game=null,roster=[],players=[],runs=[],expected=new Set(),screen='',duration=5,busy=false,scoreBusy=false,writePending=false;
  ...
  function codePanel(){return `<span class="b-eyebrow">Room code</span><strong class="b-room-display">${esc(code)}</strong><p>Open ${esc(B.title)} and enter this code.</p><div class="b-room-tools"><button class="b-secondary" data-action="copy">Copy student link</button><a class="b-secondary" target="_blank" rel="noopener" href="index.html?code=${esc(code)}">Open student screen ↗</a></div>`;}
  ...
  function draw(){
    if(!code){
      setScreen('setup',`<section class="b-setup b-panel"><img src="logo.png" alt="${esc(B.title)}"><span class="b-eyebrow">Teacher dashboard</span><h2>Open the room.</h2><p>Share one code. See who’s ready. Start when your class is settled.</p><button class="b-primary" data-action="create">Create a room</button><details><summary>Reconnect to an existing room</summary><label for="hostCode">Four-character code</label><input class="b-host-code-input" id="hostCode" type="text" maxlength="4" autocapitalize="characters" autocomplete="off" spellcheck="false" placeholder="ABCD"><button class="b-secondary" data-action="rejoin">Reconnect</button></details></section>`);return;
    }
    ...
    const p=phase();
    if(p==='idle'){
      setScreen('lobby',`<div class="b-host-grid"><section class="b-panel">${codePanel()}<label id="durationLabel">Round length</label><div class="b-duration" aria-labelledby="durationLabel">${[3,5,8,10].map(n=>`<button data-minutes="${n}" aria-pressed="${n===duration}">${n} min</button>`).join('')}</div><button class="b-primary" data-action="start">Start round</button><p class="b-host-note">Five-second countdown · individual progress · shared results</p></section><section class="b-panel"><span class="b-eyebrow">Waiting room</span><h2>Ready at their desks <span class="b-count" id="rosterCount"></span></h2><div id="roster"></div><p class="b-host-note">The roster refreshes automatically. Student IDs stay off the screen.</p></section></div>`);
  ...
  async function setupNext(){
    ...
    shell.innerHTML=`<section class="b-setup b-panel"><span class="b-eyebrow">Room ${esc(code)}</span><h2>Ready for another round?</h2><p>Students can stay in the same room.</p><label>Round length</label><div class="b-duration">${[3,5,8,10].map(n=>`<button data-minutes="${n}" aria-pressed="${n===duration}">${n} min</button>`).join('')}</div><button class="b-primary" data-action="start">Start next round</button><div class="b-actions"><button class="b-secondary" data-action="new">New room instead</button></div></section>`;screen='next';
  }
  async function act(action){
    ...
    if(action==='copy'){
      const url=new URL('index.html',location.href);url.searchParams.set('code',code);
  ...
  function soundLabel(){sound.textContent=E.audio.needsGesture()?'Enable sound':E.audio.isMuted()?'Sound off':'Sound on';sound.setAttribute('aria-pressed',!E.audio.isMuted());}
  sound.onclick=async()=>{const wasBlocked=E.audio.needsGesture();await E.audio.unlock();if(!wasBlocked)E.audio.setMuted(!E.audio.isMuted());soundLabel();};
  ...
  setInterval(()=>{if(screen!=='next'&&screen!=='podium')collect();},5000);
  const legacyHostKeys={'spot-the-error':'spoterror_host_code','double-blind':'doubleblind_host_code','skew-the-feed':'skewfeed_host_code'};
  let legacySaved='';try{legacySaved=localStorage.getItem(legacyHostKeys[B.slug])||'';}catch(_){}
  const saved=q.get('code')||E.storage.read(key,'')||legacySaved;
