...
8. **Start a different game (new code)** retires the current code and generates a fresh one — use this for a new class period. Retiring is immediate and active: students still on the old code get bounced back to registration within a poll cycle (1.5s) with "Your teacher started a new game," rather than sitting in a lobby that will never start. Starting another round on the *same* code (steps 4/6) never does this. This also retires the previous code if the dashboard had already dropped its active session.
9. If the host dashboard is reopened on another machine or after browser data is cleared, use **Already running a game? Enter its code** beneath Generate Game Code to resume that active game and its local clock.
...
4. A full class opening the page at the same moment is overloading the shared Apps Script backend (see §9). `engine.js` uses jittered self-rescheduling polls rather than overlapping intervals, and `isStaleAgainstLocalWrite` keeps an in-flight read from undoing the host's optimistic write. The host also tolerates `HOST_MISS_LIMIT` degraded null polls before it drops a session, but a genuinely overloaded backend will still be slow to give a fresh answer.

**Host dashboard dropped back to the Generate Game Code screen mid-round.** A single degraded `getGame` response used to clear the saved host code even while the round remained live. The host now shows a reconnecting note and keeps its last known game through `HOST_MISS_LIMIT` consecutive null polls; an explicitly retired code still clears immediately. If the dashboard was closed or its browser data was cleared, enter the live code in the rejoin control to resume it.

**A student sits in the lobby forever while the teacher runs a different code.** The student page correctly remains in a lobby while its code is reported alive. This happened when a host session had been torn down and its old code was left un-retired before a new code was generated; hosts now retain that previous code as a retire target. For already orphaned codes, have the student use **Nothing happening? Switch game code** and enter the current code.
...
- **Anyone who's already played a round this session always gets the on-time path**, regardless of whether the server's own 5s `countdownEndsAt` window has technically closed by the time their poll lands (`planRound(game, forceOnTime)` — `forceOnTime` is true whenever `state.joinedRound !== -1`). Poll interval + jitter + Apps Script latency can add up to nearly 5 seconds worst-case, right at the edge of the countdown window, so without this a student could occasionally get silently routed down the late-joiner path — no countdown, straight to play — for a round that, from their perspective, just started. Only a device's genuinely first-ever round join can still land the late-joiner way, which is the case that path exists for in the first place: registering mid-round, well after it started.
...
- **Host force-end** still reaches everyone: `gameEndsAt` jumps backward from what the device was told at join time, and the student page clamps its local end to it immediately rather than waiting for its next 1s tick. A 1s slack threshold keeps ordinary poll noise from tripping it. This only covers the round *shrinking in place* — see the restart case just below for the round changing entirely.
- **Mid-round restart reaches everyone too, on the next poll.** A teacher can click "End Round Now" then immediately "Start Round" — a deliberate restart, not just letting the old round run out. The new round's `gameEndsAt` isn't necessarily earlier than the old one's (it's a fresh full-length round from "now," which can land later than however much time was left on the old one), so the force-end shrink check above can't be relied on to catch it. Instead, any poll response whose `round` differs from the one a student last joined — regardless of what screen they're on, including mid-play — cancels whatever local timer/countdown/pending-answer-reveal is running and joins the new round immediately (its own 5s countdown if heard in time, straight into it if late). Without this, a student who was still actively playing when the restart happened would just finish out a round that no longer means anything to anyone else in the room.
- **Any non-play/countdown screen re-checks for a live round on every poll**, not just the lobby — `results`, `leaderboard`, wherever. Someone who clicked "View the Wire" or is still looking at their score is just as much "waiting for the next shift" as someone sitting in the lobby.
