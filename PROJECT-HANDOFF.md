...
- **A shared `engine.js`** file holds everything common to every game — student identity, game codes, the synchronized countdown/timer, and score syncing — so a second, third, or tenth game reuses it instead of rebuilding it.
...
- **`Engine.session`** — game codes and timing. `createGame()` generates a 4-character code (avoiding visually ambiguous characters: no `0/O/1/I/L`) and creates the `Games` row. `checkCode(code)` verifies a code exists. `startPolling(code, intervalMs, onUpdate)` polls `getGame` on a timer and calls `onUpdate` with the latest game object (or `null` if the code doesn't exist) — default interval is 2.5s (see §9). `startRound(code, minutes, callbacks)` and `endRound(code, callbacks)` write new timing — both also update the *caller's own* local copy of the game state immediately (before the network write even completes), so the person clicking the button sees their own dashboard react instantly rather than waiting on their own next poll. The optional `callbacks` is `{onSuccess(res), onError(err)}`, fired only after the write's own retries are exhausted one way or the other — see §9's resilience notes. `retryFailedWrite(callbacks)` resends the exact payload of the last failed start/end-round write. `derivePhase(game)` returns `"idle" | "countdown" | "live" | "ended"` purely from comparing `Date.now()` against the game's stored timestamps — this is what lets every device (including one that joins late or reloads) independently agree on the same phase without needing a push update.
...
- **`Engine.ui`** — small rendering helpers: `maskHtml(game, kicker, sub)` for the countdown overlay, `updateMaskNum(game)` to tick its number, `scoreboardTableHtml(rows, columns)` for a generic ranked table.
...
4. On the host dashboard, set the round length (minutes) and click **Start Round**. Every joined device gets a 5-second countdown mask, then plays together for the length you set — students move through the game's content at their own pace, but the clock is shared. If the write to the server can't be confirmed after retrying, the dashboard now shows a banner saying so with a **Retry** button — don't assume a round is live for students just because your own screen shows it running.
5. **End Round Now** force-ends a round early if needed.
6. Once a round ends, **Show Round Results** opens a dedicated podium-style results screen (top 3 highlighted, rest ranked below) meant to be left projected. **Start Next Round** is right there on that screen too.
...
- **Polling, not push.** Everything (game state, scoreboard) is fetched on a timer, not pushed instantly. A short delay for other devices to notice a host's action is expected and not fixable without switching to a different backend entirely (Apps Script has no realtime/push mechanism).
...
- Every request has a hard timeout (`REQUEST_TIMEOUT_MS`, currently 7s) via `AbortController`, so a hung request fails instead of leaving the UI stuck indefinitely.
- One-shot calls (student ID lookup, create game, start/end round) retry a couple of times with backoff before giving up (`withRetry`).
- The steady poll (`startPolling`) deliberately does NOT stack retries on top of a failed tick — the next tick a couple seconds later already serves as the retry, so adding real retries there would only pile more concurrent load onto a backend that's already struggling.
- `startRound`/`endRound` take an optional `{onSuccess, onError}` and only call `onError` once every retry is exhausted. `host.html` uses this to show a "couldn't confirm with the server" banner with a Retry button instead of silently assuming the write succeeded. `Engine.session.retryFailedWrite(callbacks)` resends the *exact* last failed payload rather than recomputing a fresh one, which avoids accidentally double-starting a round.
- Poll interval for both `host.html` and `index.html` was bumped from 1s to 2.5s to reduce concurrent load on the shared backend — Apps Script's own round trip is already 1-2.5s, so polling faster than that was mostly adding congestion rather than making state appear sooner.
- `host.html` now caches the last game state it heard from the server to `localStorage` and paints the dashboard from that immediately on load, instead of blocking on a bare "Loading…" until the first live poll resolves.
